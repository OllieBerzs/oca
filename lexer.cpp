...
void lex(const std::string& script, std::vector<Token>& tokens)
{
  ...
  while (index < script.size()) {
    ...
    // Keywords
      ...
      index += 2;
    } else if (match(script, index, {"end"})) {
      tokens.emplace_back(T_END, "");
      index += 3;
    }
    // Types
    else if (isOf(c, "'\""))
      tokens.emplace_back(T_STRING, scanString(script, index));
    else if (isOf(c, NUMBERS))
      tokens.emplace_back(T_NUMBER, scanNumber(script, index));
