...
#define CATCH_MINI_TESTCASE2(TestName, FunctionName, File, Line) \
    ...
    namespace { \
        bool CATCH_MINI_UNIQUE_NAME(autoReg, Line){ CatchMini::TestCase::create(&FunctionName, TestName,\
File, Line) }; \
    } \
    static void FunctionName()
...
int launchTestCases() {
    ...
    if (firstDuplicatedTestCase) {
        ...
        std::cerr << "error: TEST_CASE( " << secondDuplicatedTestCase->name << " ) already defined."
                  << std::endl;
        std::cerr << "\tFirst seen at " << firstDuplicatedTestCase->file << ':'
                  << firstDuplicatedTestCase->line << std::endl;
        std::cerr << "\tRedefined at " << secondDuplicatedTestCase->file << ':'
                  << secondDuplicatedTestCase->line << std::endl;
        exitCode = -1;
    } else {
        ...
        if (numFailedTestCases) {
            ...
            std::cout << "test cases: " << numTestCasesStr << " | " << numPassedTestCasesStr
                      << " passed | " << numFailedTestCases << " failed" << std::endl;
            std::cout << "assertions: " << numAssertionsStr << " | " << numPassedAssertionsStr
                      << " passed | " << numFailedTestCases << " failed" << std::endl;
            exitCode = 1;
        } else {
            std::cout << "All tests passed (" << numAssertions << " assertions in "
                      << testCases.size() << " test cases)" << std::endl;
            exitCode = 0;
        }
    }
...
Assertion::Assertion(const char* _expr, const char* _file, int _line) noexcept
    : expr(_expr), file(_file), line(_line) {
    ++numAssertions;
}
