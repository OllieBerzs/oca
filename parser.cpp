...
bool expr(Expression*& out, unsigned int& i, const std::vector<Token>& tokens) {
  ...
  if (tokens[i].type == T_LPAREN) {
    i++;
    if (!expr(content, i, tokens)) { return unmatch(i, orig); }
    if (tokens[i].type != T_RPAREN) { return unmatch(i, orig); }
    i++;
    out = new Expression(E_CASE, "");
  } else if (!call(out, i, tokens) && !string(out, i, tokens)
             && !number(out, i, tokens) && !boolean(out, i, tokens)) {
...
bool call(Expression*& out, unsigned int& i, const std::vector<Token>& tokens) {
  ...
  if (tokens[i].type != T_NAME) { return unmatch(i, orig); }
...
bool string(Expression*& out, unsigned int& i,
  ...
  if (tokens[i].type != T_STRING) { return unmatch(i, orig); }
...
bool number(Expression*& out, unsigned int& i,
  ...
  if (tokens[i].type != T_NUMBER) { return unmatch(i, orig); }
  std::string value = tokens[i].value;
  i++;
...
bool boolean(Expression*& out, unsigned int& i,
  ...
  if (tokens[i].type != T_BOOL) { return unmatch(i, orig); }
  std::string value = tokens[i].value;
  i++;
...
bool args(Expression*& out, unsigned int& i, const std::vector<Token>& tokens) {
  ...
  Expression* arg = nullptr;
  if (isArithmetic(tokens[i].value))
    ; // skip
...
bool block(Expression*& out, unsigned int& i,
           const std::vector<Token>& tokens) {
}
